...
Status: **implemented** 2026-09-18, all six steps (revisions
`e0dd5c5a1c06`, `dbbf7179181c`, `effa028f1a69`, `8eefb69d0194`). The
`questions` table is gone; the four-layer model is the only question store.
Rewritten 2026-09-18, replacing the 2026-09-04 draft.
...
|---|---|
| `user_id` | → `users.id` |
| `f_id` | → `fingerprints.id`, **not** `list_questions.id`. FK named `fk_attempts_f_id_fingerprints`, `ondelete=CASCADE` |
| `practiced` | |
| `updated_at` | |
| | UNIQUE `(user_id, f_id)` |
...
4. **Done 2026-09-18.** Point the API at `list_questions` joined to
   `fingerprints`, `themes` and `core_subjects`. `QuestionOut` now carries two
   ids — `id` (the list row, what the page keys on) and `f_id` (the question's
   identity) — and `occurrences` split into `month_sightings` and
   `total_sightings`. `/meta` reports `counts` (distinct questions) separately
   from `entries` (list rows), because the home page advertises a deduplicated
   archive and 3,463 Task 2 entries are only 1,512 questions. `sources` was
   dropped from `/meta`: provenance leaking into the API.
5. **Done 2026-09-18.** Migrate `attempt.question_id` → `attempt.f_id`
   (revision `effa028f1a69`). Hand-written rather than autogenerated:
   add nullable, backfill through `raw_questions`, delete unmappable rows,
   collapse rows that now collide on `(user_id, f_id)`, then tighten to NOT
   NULL with a named foreign key. Verified with seeded attempts — two rows for
   one question in different months correctly became one.
6. **Done 2026-09-18.** Drop `questions` (revision `8eefb69d0194`). The
   prerequisite was rewriting the labelling loop off it:
   `export_questions.py` now reads `fingerprints` (not `list_questions` — one
   question is one thing to label, so exporting the list layer would multiply
   the LLM bill by 2.4x and let eight copies of one question get different
   labels), and `load_labels.py` writes to `fingerprints`, resolving theme and
   core-subject names to foreign keys. It accepts either a fingerprint id or a
   13-digit scraper id, so label files made before the migration still load.
   `seed.py` and `transfer_labels.py` were deleted — the first is superseded by
   `backfill_questions.py`, the second was a one-shot that has run.

Each backfill runs against local SQLite with a dry run first, then Neon.

The one destructive step is 6, and its downgrade recreates the schema but not
the data. Dump the table first if you want the option back; the command is in
the migration's docstring.

---

## 8. The pipeline, after the migration

```text
 scrapers ──▶ questions_processing/questions_*/tache{2,3}.jsonl
                  │
                  ▼
        backfill_questions.py        raw_questions, fingerprints, list_questions
                  │                  (idempotent; ids stable across rebuilds)
                  ▼
        export_questions.py          one row per fingerprint, --unlabelled theme
                  │
                  ▼
        questions_processing/llm.py  the labelling run
                  │
                  ▼
        load_labels.py               names resolved to themes / core_subjects
```

`load_vocabulary.py --prune` keeps `themes` and `core_subjects` in step with
`llm_tasks.VOCABULARY`, which remains the single source of truth for what a
valid label is.

There is no longer any script that writes a question row by hand: every
question in the database traces back to a scraped sighting in
`raw_questions`.
...
up first. Step 5 named its own (`fk_attempts_f_id_fingerprints`); the rest
would want a `naming_convention` on `Base.metadata`, not applied yet because
it would make the existing tables' constraints differ from what the convention
describes.

**A baseline migration.** The chain still starts by altering `questions`, a
table that no longer exists in the models, so `alembic upgrade head` cannot
build a database from empty - it assumes `users` / `attempts` were created by
the `create_all` that `app/main.py` used to call. Harmless while every
database is already past that point, and the fix is one squashed revision at
the front of the chain.

**2,167 unlabelled fingerprints.** Task 3's 1,239 questions have no vocabulary
at all (`llm_tasks.VOCABULARY` is Task 2 only), and ~900 Task 2 questions have
never been through a labelling run. `export_questions.py --unlabelled theme`
is the entry point.
